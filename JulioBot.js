...
bot.command(["metar", "METAR", "Metar"], (ctx) => {
  ...
  if (userMessage.length < 4) {
    ctx.replyWithHTML(
      `<code>🎒 Tu solicitud no pudo ser procesada, recuerda que el codigo ICAO contiene 4 digitos. Ejemplo "/metar SVBS"</code>`
    );
  } else {
    ...
    fetch(url)
      ...
      .then((response) => {
        ctx.replyWithHTML(
          `<code>🤖Tu solicitud de METAR ${ctx.from.first_name} ☁️✈️:</code><code>\n${response}</code>`
        );
      });
  }
});
...
bot.command(["taf", "TAF", "Taf"], (ctx) => {
  ...
  if (userMessage.length < 4) {
    ctx.replyWithHTML(
      `<code>🎒 Tu solicitud no pudo ser procesada, recuerda que el codigo ICAO contiene 4 digitos. Ejemplo "/taf SVBS"</code>`
    );
  } else {
    ...
    fetch(url)
      ...
      .then((response) => {
        ...
        ctx.replyWithHTML(
          `<code>🤖Tu solicitud de TAF ${ctx.from.first_name} ☁️✈️:</code><code>\n${rawData}</code>`
        );
      });
  }
});
...
bot.command(["clima", "CLIMA", "Clima"], (ctx) => {
  ...
  if (userMessage.length < 4) {
    ctx.replyWithHTML(
      `<code>🎒 Tu solicitud no pudo ser procesada, por favor ingresa el nombre de la ciudad a consultar. Ejemplo "/clima Caracas"</code>`
    );
  } else {
...
bot.command(["gpt", "GPT", "Gpt"], async (ctx) => {
  await ctx.persistentChatAction("typing", async () => {
    ...
    if (userMessage.length < 4) {
      ctx.replyWithHTML(
        `<code>🎒 Tu solicitud no pudo ser procesada, por favor ingresa Una consulta valida. Ejemplo "/gpt consulta"</code>`
      );
    } else {
      ...
      const AskGPT = async () => {
        ...
        ctx.replyWithHTML(
          `<code>🎒🤖Respuesta para ${ctx.from.first_name}:</code>\n<code>${reply}</code>`
        );
      };
...
app.listen(port, () => {
  console.log(`BultoBot V-1.6.0 listening on port ${port}`);
});
